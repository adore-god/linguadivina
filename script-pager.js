(function waitForLabelsAndSchema() {
  ...
  if (!labelContainer || !map || !target) {
    setTimeout(waitForLabelsAndSchema, 100);
    return;
  }
  ...
  const updateSchema = () => {
    ...
    let graph;
    try { graph = JSON.parse(schemaScript.textContent); } 
    catch (e) { return; }
  ...
  // Use a MutationObserver in case the rest of the page content loads dynamically
  const observer = new MutationObserver((mutations, obs) => {
    const postsReady = document.getElementById("latest-posts");
    const seriesReady = document.getElementById("series-links-wrapper");
    if (postsReady && seriesReady) {
      updateSchema();
